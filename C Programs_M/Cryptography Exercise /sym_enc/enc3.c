...
//  int EVP_CipherFinal(EVP_CIPHER_CTX *ctx, unsigned char *outm, int *outl);
    ...
    // We have 5 arguments  the name of executable, the input file to encrypt, the key, the iv and the output file to store 
    // the encrypted data
    if(argc != 5){
    ...
    while((n_read = fread(buffer,1,MAX_BUFFER,f_in)) > 0){
        ...
        if(!EVP_CipherUpdate(ctx,ciphertext,&length,buffer,n_read))
            handle_errors();
        printf("length=%d\n",length);
        //write the encrypted data in the output file
    ...
    // Parameters are: the ciphertext to be written, the dimension of each element (1 Byte) the length of the ciphertext and the file to write
    if(fwrite(ciphertext,1, length, f_out) < length){
