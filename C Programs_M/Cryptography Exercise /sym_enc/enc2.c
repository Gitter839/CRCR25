...
//  int EVP_CipherFinal(EVP_CIPHER_CTX *ctx, unsigned char *outm, int *outl);
    ...
    if(strlen(argv[2])/2!=32){ // 32 B = 128 bit 
    ...
    // Copy the KEY and the IV in binary 
    ...
    while((n_read = fread(buffer,1,MAX_BUFFER,f_in)) > 0){ // We have data in the file 
        // if you risk to overflow the ciphertext variable, you exit before with an error
        // N_READ + 1 BLOC > left in cipertext (MAX - ciphertext_len)
        if(ciphertext_len > MAX_ENC_LEN - n_read - EVP_CIPHER_CTX_block_size(ctx)){ //use EVP_CIPHER_get_block_size with OpenSSL 3.0+ instead (Used in case of last block FOR PADDING due to CBC)
        ...
        if(!EVP_CipherUpdate(ctx,ciphertext+ciphertext_len,&update_len,buffer,n_read))
            handle_errors();
        ciphertext_len+=update_len; // Number of data proper processed by the context
    }
