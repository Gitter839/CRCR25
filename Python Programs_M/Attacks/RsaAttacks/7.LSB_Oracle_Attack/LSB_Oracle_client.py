...
# loop
m = ciphertext
for i in range(n.bit_length()): #For each bith (Evry bit need explcit question to the server )
    m = (pow(2, e, n) * m) % n # ((2^e mod n )* m ) mod n
...
#print decoded message (Ideal case lower bound e upper bound have the same value and this value is the message itself)
print(to_bytes(lower_bound,n.bit_length()).decode()) # the last letter is wrong, because the naive pytohn representation of integer is not precise
