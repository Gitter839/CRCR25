...
} # ',' # founde on web, frequency of each char the char e is the most frquent (py dict)
...
longest_c = max(ciphertexts, key=len) # Find the most long chipertext 
max_len = len(longest_c)
print(len(longest_c)) #Len of the longest message 

shortest_c = min(ciphertexts, key=len)
min_len = len(shortest_c)
print(len(shortest_c)) #Len of the shortes message


#################################################
# guess the first byte: naive approach: count the ascii chars (iterate for all possibile 2^8 and see the corrspondent output is a ASCII 1 btye - 256 possibile values)

counters = numpy.zeros(256,dtype=int)

for guessed_byte in range(256):
    for c in ciphertexts: #for each char of cipher text 
        if chr(c[0] ^ guessed_byte) in ascii_letters: #simulation decription, see if is an ASCII char 
            counters[guessed_byte] +=1

max_matches = max(counters) #max number reach by the counter
print(max_matches) #We have 50 ascii chars
...
for pair in ordered_match_list:
    if pair[0] < max_matches * .95: #Take canditates that has an occurence gretera or equal to max*0.95(>= 47.5 quindi 48 49 50)
        break
...
# candidates_list = []
#
# for byte_to_guess in range(min_len): # min len is the lenght of the shortetest message 
#     counters = numpy.zeros(256, dtype=int)
#
#     for guessed_byte in range(256):
#         for c in ciphertexts:
#             if chr(c[byte_to_guess] ^ guessed_byte) in printable: #instead of ascii letter, in a normal text is better tu use printable that includes all
#                 counters[guessed_byte] += 1
#
#     max_matches = max(counters)
#     # print(max_matches)
...
# #     print(c)
# All with a freq of 50, difficult to select the correct one 
#

#Try to build the keystream
# keystream = b''
# for x in candidates_list:
#     keystream += x[0][1].to_bytes(1,byteorder='big') #the most frquent of each list (the first element)
#
# from Crypto.Util.strxor import strxor
#
# for c in ciphertexts:
#     print(strxor(c[:min_len],keystream)) # Decrypt (two stream of bytes must have the same length, we use min_len to build the keystrem, so truncate the ciphertext)
# the decryption of this characrter out strange strings 


##################################################
# approach with stats (Better)

candidates_list = []

for byte_to_guess in range(max_len): #use max_len to anlyze all possibile char of ciphertexts (extend the decrypt with max_len)
    freqs = numpy.zeros(256, dtype=float)

    for guessed_byte in range(256):
        for c in ciphertexts:
            if byte_to_guess >= len(c): # ignore more lenght cipher texts 
                continue
            if chr(c[byte_to_guess] ^ guessed_byte) in printable:
                freqs[guessed_byte] += CHARACTER_FREQ.get(chr(c[byte_to_guess] ^ guessed_byte).lower(),0) #if we find in to dict get the value otherwise put 0(defualt value)
...
keystream = bytearray() # so i can modify the keystream
for x in candidates_list:
    keystream += x[0][1].to_bytes(1,byteorder='big')

from Cryptodome.Util.strxor import strxor
#Fix the inital part of senteces (blank spaces)
keystream[0] = 148 # value taken by most frequent in the naive approach

#force chars if you see there is a  sentence that starts with t e , h is missinh so al posto di questo carattere non stampabile forzo la h 
# same thing for the letter i 
dec = keystream[1] ^ ciphertexts[0][1] # get the 2 byte (position 1)  ciphertexts[0][1] is un qualunque chipertext potevo scilere un altro, tanto hanno tutti stesso problema
dec2 = keystream[2] ^ ciphertexts[0][2]#get the 3 byte

mask = dec ^ ord('h') #force h
keystream[1] = keystream[1] ^ mask
mask2 = dec2 ^ ord('i') # force i 
keystream[2] = keystream[2] ^ mask2

#you can add some fix like this char { wuold be an u -> temperat{re 


for c in ciphertexts:
    l = min(len(keystream),len(c)) # get the min between key and cipher strxor must process same length
