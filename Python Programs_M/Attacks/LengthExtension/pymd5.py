...
class md5(object):
    ...
    def __init__(self, string='', state=None, count=0): 
        ...
        if state is None:
            # initial state defined by standard (init string for the computation process)
...
def _decode(input, len):
    k = len // 4 # from string to a sequence of data 
...
if __name__ == '__main__':
    ...
    new_hash_gen = md5(state=sniffed_keyed_digest,count= 2*md5.block_size *8) # counter for the padding , state use the sniffed digest to perform the new compuatation
    ...
    print(hg.hexdigest())
    real_message = message + pad + to_add
    print(real_message)